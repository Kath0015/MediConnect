...
class SendAppointmentCreatedNotification
{
    ...
    {
        $appointment = $event->appointment->load(['patient.user', 'appointmentType', 'clinician']);
        $patientEmail = $appointment->patient?->user?->email;

        if (!$patientEmail) {
            Log::warning('Cannot send appointment created email: patient email not found', [
        ...
        try {
            ...
            if (!$mailSettings) {
                Log::warning('Mail settings not configured in database for appointment creation notification');
                return;
            }
            ...
            Log::info('Sending appointment created email', [
                'appointment_id' => $appointment->id,
            ...
            Mail::to($patientEmail)->send(new AppointmentCreatedMail($appointment));

            Log::info('Appointment created email sent successfully', [
                'appointment_id' => $appointment->id,
                'patient_email' => $patientEmail,
            ]);
        } catch (\Exception $e) {
