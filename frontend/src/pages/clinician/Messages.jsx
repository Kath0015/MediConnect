...
import { MessageCircle, Send, Search, Plus, Loader2, Users, Stethoscope } from 'lucide-react';
...
const ClinicianMessages = () => {
  ...
  const getRoleIcon = (roleKey) => {
    switch (roleKey) {
      case 'doctor':
        return <Stethoscope className="w-3 h-3 text-[#26B170]" />;
      case 'patient':
        return <Users className="w-3 h-3 text-blue-500" />;
      case 'admin':
        return <Users className="w-3 h-3 text-purple-500" />;
      default:
        return <Users className="w-3 h-3 text-[#26B170]" />;
    }
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1 text-sm">
            Communicate with doctors, patients, and administration.
          </p>
        </div>
        <button
          onClick={() => {
          ...
          className="flex items-center gap-2 px-4 py-2 bg-[#26B170] hover:bg-[#1e8a5a] text-white text-sm font-medium rounded-xl transition-all shadow-sm cursor-pointer"
        >
          ...
          <div className="p-3 border-b border-slate-100 space-y-2">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#26B170]/30 focus:border-[#26B170]"
              />
            </div>
            {showNewChat && (
          ...
          <div className="flex-1 overflow-y-auto divide-y divide-slate-100">
            {showNewChat ? (
              filteredContacts.length === 0 ? (
                ...
                filteredContacts.map((contact) => (
                  <button
                    ...
                    className="w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-emerald-50/50 transition-colors cursor-pointer"
                  >
              ...
              <div className="p-8 text-center text-slate-400 text-sm flex items-center justify-center gap-2">
                <Loader2 className="w-4 h-4 animate-spin text-[#26B170]" />
              ...
              <div className="p-8 text-center text-slate-400 text-sm space-y-2">
                ...
                <button
                  ...
                  className="text-xs text-[#26B170] font-semibold hover:underline cursor-pointer"
                >
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-3 text-left hover:bg-emerald-50/50 transition-colors cursor-pointer ${
                    selectedUser?.id === c.id
                      ? 'bg-[#26B170]/5 border-l-4 border-[#26B170]'
                      : ''
                  }`}
                >
                  <div className="w-10 h-10 rounded-full bg-[#26B170] text-white flex items-center justify-center text-sm font-bold flex-shrink-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center justify-between mt-0.5">
                      ...
                      {c.unread > 0 && (
                        <span className="bg-[#26B170] text-white text-[10px] font-bold min-w-4 h-4 px-1 rounded-full flex items-center justify-center flex-shrink-0 ml-1">
                          {c.unread}
                        </span>
                      )}
                    </div>
        ...
        <div className="flex-1 bg-white rounded-2xl border border-slate-100 shadow-sm flex flex-col overflow-hidden">
          {selectedUser ? (
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-3 bg-slate-50/40">
                {loadingMessages ? (
                  <div className="h-full flex items-center justify-center text-slate-400 text-sm gap-2">
                    <Loader2 className="w-5 h-5 animate-spin text-[#26B170]" />
                    Loading messages...
                  </div>
                ) : messages.length === 0 ? (
                    ...
                    >
                      <div
                        className={`max-w-md rounded-2xl px-4 py-2.5 text-sm shadow-xs ${
                          m.mine
                            ? 'bg-[#26B170] text-white rounded-tr-xs'
              ...
              <div className="p-4 border-t border-slate-100 bg-white">
                <form onSubmit={handleSend} className="flex items-center gap-3">
                  <input
                    ...
                    disabled={sending}
                    className="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-[#26B170]/30 focus:border-[#26B170] transition-all"
                  />
                  <button
                    type="submit"
                    disabled={!input.trim() || sending}
                    className="w-10 h-10 rounded-xl bg-[#26B170] text-white flex items-center justify-center hover:bg-[#1e8a5a] transition-all disabled:opacity-50 shadow-sm cursor-pointer"
                  >
                    {sending ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-slate-400 p-8 text-center">
              ...
              <h3 className="font-semibold text-slate-700">Staff Messaging</h3>
              <p className="text-sm mt-1 max-w-sm">
                Click "New Message" to start communicating with doctors, patients, or administration.
              </p>
            </div>
          )}
        </div>
